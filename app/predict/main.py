...
def predict_traffic(db: Session, test_mode: bool = False, test_time: datetime = None):
    try:
        ...
        all_combinations = pd.DataFrame(
            list(product(time_series_sorted['Time'], df_mapping['link_id'])),
            columns=['tm', 'link_id']
        )
        complete_df = all_combinations.merge(df_mapping, on='link_id', how='left')
        ...
        # 결측치 최종 확인
        missing_speed = complete_df['speed'].isna().sum()
        missing_pty = complete_df['pty'].isna().sum()
        missing_rn1 = complete_df['rn1'].isna().sum()
        if missing_speed > 0:
            logging.warning(f"Missing traffic data: {missing_speed} records")
        if missing_pty > 0 or missing_rn1 > 0:
            logging.warning(f"Missing values found - PTY: {missing_pty}, RN1: {missing_rn1}")

        complete_df['pty'] = complete_df['pty'].fillna(0)
        complete_df['rn1'] = complete_df['rn1'].fillna(0)
        ...
        result = U.process_data(complete_df, weakday, time_series_sorted)
...
if __name__ == '__main__':
    ...
    # 테스트 실행을 위한 시간 설정
    test_time = datetime(2025, 1, 23, 14, 0, 0, tzinfo=ZoneInfo("Asia/Seoul"))
